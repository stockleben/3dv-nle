...
$(document).ready(function() {
	...
	$("#canvas_div").mousedown(function(e) {
		...
		$('#position').html(e.pageX + ', ' + e.pageY + "isInside:"+action+"<br/>"+box.toString());
		if (action){
	...
	$("#markers").mouseup(function(e){
		if (dragging && dragging_box != null){
			$('#markers').append('<br/><a href="'+dragging_box.link+'" target="_blank">'+dragging_box.link+'</a>');
		}
		dragging = false;
		dragging_box = null;
		$('#canvas_div').css('cursor','default');
...
function BoxObject(x1, y1, x2, y2, link, link_id) {
	...
	function toString(){
		return "BoxObject "+x1+","+y1+","+x2+","+y2+" link:"+link+" link_id:"+link_id;
	}
	
	this.getCopy = getCopy;
	function getCopy(){
		return new BoxObject(this.x1,this.y1,this.x2,this.y2,this.link,this.link_id);
	}
}
			...
			function() {
				...
				pop.code({
					...
					onStart : function(options) {
						...
						box = new BoxObject(x1, y1, x2, y2, destination, object_id);
